  ...
  return (
    <div className="bg-background min-h-screen pb-24">
      ...
      <div className="max-w-[1400px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 mb-16">
        {/* Left: Images */}
        <div>
          <div className="aspect-[4/3] bg-card rounded-xl mb-4 border border-border flex items-center justify-center relative overflow-hidden shadow-sm">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[1,2,3,4].map(i => (
              <div key={i} className={`aspect-[4/3] bg-card rounded-lg cursor-pointer transition-all border-2 ${i === 1 ? 'border-green-700' : 'border-transparent hover:border-foreground/20'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="py-2">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex items-center gap-1 text-sm font-medium text-foreground/70">
              ...
              <span className="text-foreground/40">(2,140 Reviews)</span>
            </div>
          </div>

          <h1 className="text-4xl font-bold text-foreground tracking-tight mb-4">{product.name}</h1>
          ...
          <div className="flex items-end gap-4 mb-2">
            ...
            <span className="text-lg font-medium text-foreground/40 line-through mb-1">₹{Math.round(product.price * 1.2)}</span>
            <span className="px-2 py-1 bg-green-700 text-white text-[10px] font-bold tracking-wider rounded mb-1.5">-20% OFF</span>
          </div>
          <p className="text-xs text-foreground/60 mb-8">Free shipping on orders over ₹1000</p>

          <div className="mb-8">
            <h3 className="text-[11px] font-bold text-foreground/40 tracking-wider uppercase mb-3">Select Flavor</h3>
            <div className="flex gap-3">
              <button className="px-4 py-2 border-2 border-green-700 text-sm font-medium text-foreground rounded-md bg-green-50">Unflavored</button>
              <button className="px-4 py-2 border-2 border-transparent bg-card shadow-sm hover:border-foreground/20 text-sm font-medium text-foreground/70 rounded-md transition-colors">Rich Cacao</button>
              <button className="px-4 py-2 border-2 border-transparent bg-card shadow-sm hover:border-foreground/20 text-sm font-medium text-foreground/70 rounded-md transition-colors">Vanilla Bean</button>
            </div>
          </div>
          ...
          <div className="flex gap-4 mb-8 mt-4">
            <button className="flex-1 flex items-center justify-center border border-border bg-card rounded-lg hover:border-green-600 hover:text-green-600 transition-colors text-foreground/40 py-3">
              <Heart className="w-5 h-5 mr-2" /> Wishlist
            </button>
            <button className="flex-1 flex items-center justify-center border border-border bg-card rounded-lg hover:border-foreground transition-colors text-foreground/40 py-3">
              <Share2 className="w-5 h-5 mr-2" /> Share
            </button>
          </div>

          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex items-center gap-2 text-xs font-medium text-foreground/70 bg-card px-3 py-2 border border-border rounded-md">
      ...
      <div className="max-w-[1400px] mx-auto px-6 mb-24">
        <div className="flex gap-8 border-b border-border mb-8">
          ...
          <button className="pb-3 text-sm font-medium text-foreground/60 hover:text-foreground transition-colors">Ingredients & Macros</button>
          <button className="pb-3 text-sm font-medium text-foreground/60 hover:text-foreground transition-colors">Reviews (2,140)</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[
            ...
            <div key={i} className="bg-card p-6 rounded-xl border border-border flex gap-4">
      ...
      <div className="max-w-[1400px] mx-auto px-6">
        <div className="flex justify-between items-end mb-8">
          <h2 className="text-2xl font-bold text-foreground">Explore the Collection</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
          {products.filter(p => p.id !== id).slice(0, 4).map(p => (
            <Link href={`/product/${p.id}`} key={p.id} className="group flex flex-col">
              <div className="aspect-[4/3] bg-card border border-border rounded-xl mb-4 overflow-hidden relative p-4 flex items-center justify-center">
                <div className="w-2/3 h-full bg-gradient-to-tr from-[#f4f7f5] to-[#e8f0ea] border border-border rounded-lg group-hover:scale-105 transition-transform" />
              </div>
              <h3 className="font-semibold text-foreground text-sm mb-1 truncate">{p.name}</h3>
              <p className="text-foreground/60 text-xs font-medium">₹{p.price}</p>
            </Link>
          ))}
        </div>
      </div>
